...
void StreamFinderPlugin::RenderSettings() {
	...
	if (ImGui::Button("Stream Finder")) {
		gameWrapper->Execute([this](GameWrapper* gw) { cvarManager->executeCommand("StreamFinder"); });
	}
	...
	std::ofstream webhook(gameWrapper->GetDataFolder() / "StreamFinder" / "discord-webhook.txt");
